...
import { HOUSE_CLEANING_CITIES } from "@/content/service-city";

export function generateStaticParams() {
  return HOUSE_CLEANING_CITIES.map((p) => ({ city: p.city }));
}
...
export async function generateMetadata({
  ...
  const page = HOUSE_CLEANING_CITIES.find((p) => p.city === city);
  if (!page) return {};
  return generatePageMetadata({
    title: page.metaTitle,
    description: page.metaDescription,
    path: `/house-cleaning/${city}`,
  });
}

export default async function HouseCleaningCityPage({
  params,
}: {
  params: Promise<{ city: string }>;
}) {
  ...
  const serviceSchema = {
    ...
    "@type": "Service",
    name: `House Cleaning Services in ${page.cityName}, ${page.province}`,
    description: page.metaDescription,
    url: `${SITE_CONFIG.url}/house-cleaning/${city}`,
    provider: {
      "@type": "LocalBusiness",
    ...
    <>
      ...
      <JsonLd
        data={generateBreadcrumbSchema([
          ...
          { name: `House Cleaning in ${page.cityName}`, url: `/house-cleaning/${city}` },
        ])}
      />
      ...
      <div className="max-w-4xl mx-auto px-4 py-12 md:py-16 space-y-12">
        ...
        <section className="bg-primary/5 border border-primary/20 rounded-xl p-6 md:p-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-3">
            House Cleaning Prices in {page.cityName}
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
            <div className="text-center p-4 bg-white rounded-lg border">
              <p className="text-sm text-gray-500 mb-1">Apartment</p>
              <p className="text-2xl font-bold text-primary">$99+</p>
              <p className="text-xs text-gray-500">$33/hr</p>
            </div>
            <div className="text-center p-4 bg-white rounded-lg border">
              <p className="text-sm text-gray-500 mb-1">Townhouse</p>
              <p className="text-2xl font-bold text-primary">$132+</p>
              <p className="text-xs text-gray-500">$33/hr, 4hr min</p>
            </div>
            <div className="text-center p-4 bg-white rounded-lg border">
              <p className="text-sm text-gray-500 mb-1">House (3+ bed)</p>
              <p className="text-2xl font-bold text-primary">$165+</p>
              <p className="text-xs text-gray-500">$33/hr</p>
            </div>
          </div>
          <p className="text-sm text-gray-500 mb-4">All supplies included. Weekly 15% off · Bi-weekly 10% off · Monthly 5% off.</p>
          <Link
            href="/pricing"
            className="inline-flex items-center gap-2 bg-primary text-white px-5 py-2.5 rounded-lg font-semibold hover:bg-primary-dark transition-colors text-sm"
          >
        ...
        <section>
          <h2 className="text-2xl font-bold text-gray-900 mb-4">
            House Cleaning FAQ — {page.cityName}
          </h2>

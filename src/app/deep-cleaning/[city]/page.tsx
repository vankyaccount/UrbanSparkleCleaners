...
import { DEEP_CLEANING_CITIES } from "@/content/service-city";

export function generateStaticParams() {
  return DEEP_CLEANING_CITIES.map((p) => ({ city: p.city }));
}
...
export async function generateMetadata({
  ...
  const page = DEEP_CLEANING_CITIES.find((p) => p.city === city);
  if (!page) return {};
  return generatePageMetadata({
    title: page.metaTitle,
    description: page.metaDescription,
    path: `/deep-cleaning/${city}`,
  });
}

export default async function DeepCleaningCityPage({
  params,
}: {
  params: Promise<{ city: string }>;
}) {
  ...
  const serviceSchema = {
    ...
    "@type": "Service",
    name: `Deep Cleaning Services in ${page.cityName}, ${page.province}`,
    description: page.metaDescription,
    url: `${SITE_CONFIG.url}/deep-cleaning/${city}`,
    provider: {
      "@type": "LocalBusiness",
    ...
    <>
      ...
      <JsonLd
        data={generateBreadcrumbSchema([
          ...
          { name: `Deep Cleaning in ${page.cityName}`, url: `/deep-cleaning/${city}` },
        ])}
      />
      ...
      <div className="max-w-4xl mx-auto px-4 py-12 md:py-16 space-y-12">
        ...
        <section className="bg-primary/5 border border-primary/20 rounded-xl p-6 md:p-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-3">
            Deep Cleaning Prices in {page.cityName}
          </h2>
          <p className="text-sm text-gray-600 mb-4">$40/hour — minimum 2 technicians. All supplies included.</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
            <div className="text-center p-4 bg-white rounded-lg border">
              <p className="text-sm text-gray-500 mb-1">Apartment</p>
              <p className="text-2xl font-bold text-primary">$120+</p>
              <p className="text-xs text-gray-500">$40/hr, 2 techs</p>
            </div>
            <div className="text-center p-4 bg-white rounded-lg border">
        ...
        <section>
          <h2 className="text-2xl font-bold text-gray-900 mb-4">
            Deep Cleaning FAQ — {page.cityName}
          </h2>
